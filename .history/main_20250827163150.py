    ...
    if st.session_state.daily_data:
        ...
        if view_mode == "📋 一覧表示":
            ...
            delete_indices = st.multiselect(
                ...
                format_func=lambda x: f"{st.session_state.daily_data[x]['時刻']} | {st.session_state.daily_data[x]['入力者']} | {st.session_state.daily_data[x]['店舗名']} | {st.session_state.daily_data[x]['金額']:,}円"
            )
            
            if delete_indices and st.button("🗑️ 選択した項目を削除", key="delete_selected"):
