...
with tab2:
    ...
    if st.session_state.daily_data:
        ...
        for i, entry in enumerate(st.session_state.daily_data):
            ...
            with col1:
                st.write(f"**{entry['時刻']}** | {entry['入力者']} | {entry['店舗名']} | {entry['内容']} | **{entry['金額']:,}円**")
            with col2:
        ...
        if not df_today.empty:
            ...
            with col2:
                # 入力者毎の店舗集計
                user_store_summary = df_today.groupby(['入力者', '店舗名'])['金額'].sum().reset_index()
                user_store_summary = user_store_summary.sort_values(['入力者', '金額'], ascending=[True, False])
                st.subheader("👤 入力者毎の店舗集計")
                st.dataframe(user_store_summary)
    ...
    if st.session_state.saved_daily_data:
        ...
        if selected_date and selected_date in st.session_state.saved_daily_data:
            ...
            if not df_past.empty:
                ...
                with col2:
                    # 入力者毎の店舗集計
                    user_store_summary_past = df_past.groupby(['入力者', '店舗名'])['金額'].sum().reset_index()
                    user_store_summary_past = user_store_summary_past.sort_values(['入力者', '金額'], ascending=[True, False])
                    st.subheader("👤 入力者毎の店舗集計")
                    st.dataframe(user_store_summary_past)
