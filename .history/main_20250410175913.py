...
def get_shop_urls():
    ...
    try:
        ...
        print(f"取得したURLの総数: {len(shop_urls)}")
...
def get_shop_info(shop_url):
    ...
    try:
        ...
        # システム情報を取得（システムページにアクセス）
        system_url = shop_url.replace('/index.html', '/system.html')
        print(f"システムページにアクセス: {system_url}")
...
def scrape_host_clubs():
    ...
    clubs = []
    total_urls = len(shop_urls)
    for i, url in enumerate(shop_urls, 1):
        print(f"\n処理中: {i}/{total_urls}")
        shop_info = get_shop_info(url)
        if shop_info:
            clubs.append(shop_info)
            print(f"成功: {i}/{total_urls} ({len(clubs)}件取得済み)")
        else:
            print(f"失敗: {i}/{total_urls}")
