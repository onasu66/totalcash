...
with tab2:
    ...
    # 店舗名と金額入力
    store_name = st.text_input("店舗名（例: DNA）", key="single_store")
    content_input = st.text_input("金額・バック（例: 1.1000.⭕️）", key="single_content")
    
    # 処理ボタン
    if st.button("➕ この1回分を追加", key="add_single_talk"):
        if single_talk_input.strip():
            # 1回分のトーク履歴を解析
            lines = single_talk_input.splitlines()
            ...
            for i in indices:
                ...
                while j < len(lines) and not re.match(r'\d{1,2}:\d{2}', lines[j]):
                    ...
                    if is_money_line and current_store:
            ...
            for i, line in enumerate(lines):
                if re.match(r'\d{1,2}:\d{2}', line) and not pattern.search(line):
                    ...
                    if time_content_match:
                        ...
                        parts = content_after_time.split(None, 1)
                    ...
                    j = i + 1
                    current_store = initial_store
                    
                    while j < len(lines) and not re.match(r'\d{1,2}:\d{2}', lines[j]):
                        content_line = lines[j].strip()
                        
                        if not content_line or "メッセージの送信を取り消しました" in content_line:
                            j += 1
                            continue
                        ...
                        elif not is_money_line and j + 1 < len(lines):
                            next_line_without_time = re.sub(r'\d{1,2}:\d{2}', '', lines[j + 1])
                            next_is_money = any(re.search(pattern, next_line_without_time) for pattern in money_patterns)
                            if next_is_money:
            ...
            # データを累積に追加
            if talk_data:
                st.session_state.daily_data.extend(talk_data)
                st.success(f"✅ {len(talk_data)}件のデータを追加しました")
            else:
                st.warning("このトーク履歴からはデータを抽出できませんでした")
        else:
            st.warning("トーク履歴を入力してください")
    
    # 今日の累積データ表示
    if st.session_state.daily_data:
        st.subheader(f"📅 今日の累積データ ({st.session_state.today_date})")
        ...
        with col1:
            if st.button("🗑️ 今日のデータをリセット", key="reset_today"):
                ...
                st.rerun()
        with col2:
            if st.button("💾 Excelで保存", key="save_excel"):
                # 簡単なExcel保存機能
        ...
        st.info("まだデータがありません。上記から1回分のトーク履歴を入力してください。")
