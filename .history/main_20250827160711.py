        ...
        if view_mode == "📋 一覧表示":
            ...
            for i in reversed(range(len(st.session_state.daily_data))):
                ...
                with st.container():
                    ...
                    with col1:
                        st.markdown(f"""
                        **{entry['時刻']}** | **{entry['入力者']}** | {entry['店舗名']} | {entry['内容']} | **{entry['金額']:,}円**
                        """)
                    with col2:
                        if st.button("🗑️", key=f"delete_{i}", help="この項目を削除"):
    ...
    if st.session_state.saved_daily_data:
        ...
        if selected_date and selected_date in st.session_state.saved_daily_data:
            ...
            if not df_past.empty:
                ...
                st.subheader("🏪 店舗別合計")
                st.dataframe(store_summary_past, use_container_width=True)
                
                # 入力者毎の店舗集計（タブ化）
                st.subheader("👤 入力者毎の詳細")
                
                # ユニークな入力者を取得
                users_past = df_past['入力者'].unique()
                
                if len(users_past) > 0:
                    # タブを作成
                    user_tabs_past = st.tabs([f"👤 {user}" for user in users_past])
                    
                    for i, user in enumerate(users_past):
                        with user_tabs_past[i]:
                            # その入力者のデータをフィルタ
                            user_data_past = df_past[df_past['入力者'] == user]
                            
                            # 店舗名と内容のみを表示
                            display_data_past = user_data_past[['店舗名', '内容']].copy()
                            
                            st.write(f"**{user}さんの入力内容**")
                            st.dataframe(display_data_past, use_container_width=True)
                            
                            # その人の合計金額
                            user_total_past = user_data_past['金額'].sum()
                            st.metric(f"{user}さんの合計", f"{user_total_past:,}円")
        ...
        with col1:
            if st.button("💾 全期間をExcel一括保存", key="save_all_excel"):
                ...
                if all_data:
                    ...
                    # 列の順序を調整
                    columns_order = ["営業日", "時刻", "入力者", "店舗名", "内容", "金額"]
